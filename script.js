...
async function loadProducts() {
    try {
        const response = await fetch('https://webbackend-production-edfc.up.railway.app/api/products');
        if (!response.ok) {
...
async function goToProductDetails(productId) {
    try {
        ...
        const resp = await fetch(`https://webbackend-production-edfc.up.railway.app/api/products/${productId}`);
        if (resp.ok) {
